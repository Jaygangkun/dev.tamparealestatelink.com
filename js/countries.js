/////////////////////////////////////////////////////////////////////
// COUNTRIES AND STATE SELECTION FUNCTION                          //
//                                                                 //
...
$(function()
{
 var selected_val_country = $("#country option:selected").val();
 var html_country = '';
 var selected;
 for(var i in country_arr)
 {
  selected = (country_arr[i]==selected_val_country) ? ' SELECTED' : '';
  if(country_arr[i]==selected_val_country) { showStates(country_arr[i]); }
  html_country += '<option value="'+country_arr[i]+'"'+selected+'>'+country_arr[i]+'</option>';
 }
 $('.country_select').html('<select id="country" name="country">'+html_country+'</select>');
 $('select').selectbox();
 fixSelectBoxZindex();
 $('#country').change(function() { showStates($('#country option:selected').val()); });
});
function showStates(country)
{
 var selected_val_state = $("#state option:selected").val();
 var html_state = '';
 var selected;
 var states;
 for(var i in country_arr)
 {
  ...
  {
   states = s_a[(i*1+1)].split('|');
   for(var n in states)
   {
    selected = (states[n]==selected_val_state) ? ' SELECTED' : '';
    html_state += '<option value="'+states[n]+'"'+selected+'>'+states[n]+'</option>';
   }
   if(states.lenth==0) {  }
   break;
  }
 }
 html = (states.lenth==0) ? '<input type="text" name="state" value="'+selected_val_state+'">' : '<select id="state" name="state">'+html_state+'</select>';
